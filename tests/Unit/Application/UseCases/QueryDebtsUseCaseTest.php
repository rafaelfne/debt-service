...
it('produces the enunciado canon for the combined IPVA + MULTA list', function (): void {
    ...
    expect($result)->toBeInstanceOf(DebtQueryResult::class)
        ...
        ->and($result->debts[0]->updatedAmount->toString())->toBe('1800.40')
        ...
        ->and($result->summary->totalOriginal->toString())->toBe('1800.83')
        ->and($result->summary->totalUpdated->toString())->toBe('2356.32')
        ->and($result->paymentOptions)->toHaveCount(3)
        ->and($result->paymentOptions[0]->type)->toBe('TOTAL')
        ->and($result->paymentOptions[0]->baseAmount->toString())->toBe('2356.32')
        ->and($result->paymentOptions[1]->type)->toBe('SOMENTE_IPVA')
        ->and($result->paymentOptions[1]->baseAmount->toString())->toBe('1800.40')
