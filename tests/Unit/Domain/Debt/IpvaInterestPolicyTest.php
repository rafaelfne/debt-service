...
it('returns the 20% cap when raw interest exceeds it (canon: 1500.00 × 121d → 300.00)', function (): void {
    $interest = $this->policy->calculate(Money::of('1500.00'), 121);

    expect($interest->toString())->toBe('300.00')
        ->and($interest->equals(Money::of('300.00')))->toBeTrue();
});

it('returns zero when daysOverdue is 0 (canon: 1500.00 × 0d → 0.00)', function (): void {
    expect($this->policy->calculate(Money::of('1500.00'), 0)->toString())->toBe('0.00');
});

it('returns the raw interest below the cap (canon: 1500.00 × 30d → 148.50)', function (): void {
    expect($this->policy->calculate(Money::of('1500.00'), 30)->toString())->toBe('148.50');
});

it('returns zero for negative daysOverdue', function (): void {
    expect($this->policy->calculate(Money::of('1500.00'), -5)->toString())->toBe('0.00');
});
